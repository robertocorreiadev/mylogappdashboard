...
const STATUS_COLORS: Record<string, string> = {
  pendente: "var(--chart-5)", em_transito: "var(--chart-4)",
  entregue: "var(--chart-2)", cancelada: "var(--chart-3)",
}
const MONTHS = ["Jan","Fev","Mar","Abr","Mai","Jun","Jul","Ago","Set","Out","Nov","Dez"]

export function OverviewPanel({ deliveries, transactions, dailyRecords }: {
  deliveries: Delivery[]; transactions: Transaction[]; dailyRecords: DailyRecord[]
}) {
  const statusData = Object.entries(
    deliveries.reduce<Record<string, number>>((acc, d) => { acc[d.status] = (acc[d.status] ?? 0) + 1; return acc }, {})
  ...
  for (const t of transactions) {
    ...
    const key = `${d.getFullYear()}-${d.getMonth()}`
    finMap[key] = finMap[key] ?? { receita: 0, despesa: 0 }
    ...
    <div className="grid gap-4 lg:grid-cols-2">
      <Card>
        ...
        <CardContent>
          {boletaData.length === 0
            ...
            : <ChartContainer config={{ bruto: { label: "Bruto", color: "var(--chart-1)" }, liquido: { label: "Líquido", color: "var(--chart-2)" }, despesas: { label: "Despesas", color: "var(--chart-3)" } }} className="max-h-[260px] w-full">
                <BarChart data={boletaData}>
                  <CartesianGrid vertical={false} strokeDasharray="3 3" />
                  <XAxis dataKey="mes" tickLine={false} axisLine={false} tickMargin={8} />
                  <YAxis tickLine={false} axisLine={false} width={55} tickFormatter={v => `R$${(Number(v)/1000).toFixed(0)}k`} />
      ...
      {finData.length > 0 && (
        <Card className="lg:col-span-2">
          ...
          <CardContent>
            <ChartContainer config={{ receita: { label: "Receita", color: "var(--chart-2)" }, despesa: { label: "Despesa", color: "var(--chart-3)" } }} className="max-h-[220px] w-full">
