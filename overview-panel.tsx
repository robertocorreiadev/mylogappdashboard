...
export function OverviewPanel({
  ...
  for (const r of dailyRecords) {
    ...
    boletaMap[key].bruto    += gross
    boletaMap[key].despesas += Number(r.expenses)
    boletaMap[key].liquido  += gross - Number(r.expenses)
  }
  const boletaData = Object.entries(boletaMap)
    .map(([key, v]) => {
  ...
  // Gráfico financeiro (transações)
  const financeMap: Record<string, { receita: number; despesa: number }> = {}
  for (const t of transactions) {
    const d = new Date(t.date + "T00:00:00")
    const key = `${d.getFullYear()}-${d.getMonth()}`
    financeMap[key] = financeMap[key] ?? { receita: 0, despesa: 0 }
    if (t.type === "receita") financeMap[key].receita += Number(t.amount)
    else financeMap[key].despesa += Number(t.amount)
  }
  const financeData = Object.entries(financeMap)
    .map(([key, v]) => {
      const [year, month] = key.split("-").map(Number)
      return { sort: year * 12 + month, mes: `${MONTHS[month]}/${String(year).slice(2)}`, ...v }
    })
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-2">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">Boletas mensais (bruto × líquido)</CardTitle>
        </CardHeader>
        <CardContent>
          {boletaData.length === 0 ? (
            ...
            >
              <BarChart data={boletaData}>
                ...
                <YAxis tickLine={false} axisLine={false} width={50} tickFormatter={(v) => `R$${(v/1000).toFixed(0)}k`} />
      ...
      {/* Status das entregas rastreadas */}
      <Card>
        <CardHeader>
          <CardTitle className="text-base">Entregas por status (rastreio)</CardTitle>
        </CardHeader>
        <CardContent>
          {statusData.length === 0 ? (
      ...
      {financeData.length > 0 && (
        <Card className="lg:col-span-2">
          <CardHeader>
            <CardTitle className="text-base">Receitas × Despesas (financeiro)</CardTitle>
          </CardHeader>
          <CardContent>
            <ChartContainer
              config={{
