  ...
  const cards = [
    {
      ...
      hint: `${workDays} dias trabalhados`,
      icon: Truck,
      color: "text-primary",
    },
    {
      label: "Fat. bruto (boletas)",
      value: formatCurrency(totalGross),
      hint: `Despesas: ${formatCurrency(totalExpenses)}`,
      icon: TrendingUp,
      color: "text-[var(--chart-2)]",
    },
    {
      label: "Fat. líquido (boletas)",
      value: formatCurrency(totalNet),
      hint: `Média diária: ${formatCurrency(avgNet)}`,
      icon: Wallet,
      color: totalNet >= 0 ? "text-[var(--chart-2)]" : "text-destructive",
    },
    {
      label: "Entregas (rastreio)",
      value: String(deliveries.length),
      hint: `${deliveries.filter((d) => d.status === "pendente" || d.status === "em_transito").length} em aberto`,
      icon: Package,
